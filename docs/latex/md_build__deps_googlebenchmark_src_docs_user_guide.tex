\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md33}{}\doxysection{Command Line}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md33}
...
\href{\#extra-context}{\texttt{ Extra Context}}\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md34}{}\doxysection{Library}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md34}
...
\href{\#cpu-timers}{\texttt{ CPU Timers}}

\href{\#manual-timing}{\texttt{ Manual Timing}}

\href{\#setting-the-time-unit}{\texttt{ Setting the Time Unit}}

\mbox{\hyperlink{md_build__deps_googlebenchmark_src_docs_random_interleaving}{Random Interleaving}}

\mbox{\hyperlink{md_build__deps_googlebenchmark_src_docs_perf_counters}{User-\/\+Requested Performance Counters}}
...
\href{\#memory-usage}{\texttt{ Memory Usage}}

\href{\#using-register-benchmark}{\texttt{ Using Register\+Benchmark}}

\href{\#exiting-with-an-error}{\texttt{ Exiting with an Error}}

\href{\#a-faster-keep-running-loop}{\texttt{ A Faster {\ttfamily Keep\+Running} Loop}}\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md35}{}\doxysection{Benchmarking Tips}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md35}
\href{\#disabling-cpu-frequency-scaling}{\texttt{ Disabling CPU Frequency Scaling}}

\mbox{\hyperlink{md_build__deps_googlebenchmark_src_docs_reducing_variance}{Reducing Variance in Benchmarks}}

\label{_output-formats}%
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md36}{}\doxysection{Output Formats}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md36}
The library supports multiple output formats. Use the {\ttfamily -\/-\/benchmark\+\_\+format=$<$console$\vert$json$\vert$csv$>$} flag (or set the {\ttfamily BENCHMARK\+\_\+\+FORMAT=$<$console$\vert$json$\vert$csv$>$} environment variable) to set the format type. {\ttfamily console} is the default format.
...
The JSON format outputs human readable json split into two top level attributes. The {\ttfamily context} attribute contains information about the run in general, including information about the CPU and the date. The {\ttfamily benchmarks} attribute contains a list of every benchmark run. Example json output looks like\+:
...
The CSV format outputs comma-\/separated values. The {\ttfamily context} is output on stderr and the CSV itself on stdout. Example CSV output looks like\+:
...
\label{_output-files}%
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md37}{}\doxysection{Output Files}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md37}
Write benchmark results to a file with the {\ttfamily -\/-\/benchmark\+\_\+out=$<$filename$>$} option (or set {\ttfamily BENCHMARK\+\_\+\+OUT}). Specify the output format with {\ttfamily -\/-\/benchmark\+\_\+out\+\_\+format=\{json$\vert$console$\vert$csv\}} (or set {\ttfamily BENCHMARK\+\_\+\+OUT\+\_\+\+FORMAT=\{json$\vert$console$\vert$csv\}}). Note that the \textquotesingle{}csv\textquotesingle{} reporter is deprecated and the saved {\ttfamily .csv} file \href{https://github.com/google/benchmark/issues/794}{\texttt{ is not parsable}} by csv parsers.

Specifying {\ttfamily -\/-\/benchmark\+\_\+out} does not suppress the console output.

\label{_running-benchmarks}%
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md38}{}\doxysection{Running Benchmarks}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md38}
Benchmarks are executed by running the produced binaries. Benchmarks binaries, by default, accept options that may be specified either through their command line interface or by setting environment variables before execution. For every {\ttfamily -\/-\/option\+\_\+flag=$<$value$>$} CLI switch, a corresponding environment variable {\ttfamily OPTION\+\_\+\+FLAG=$<$value$>$} exist and is used as default if set (CLI switches always prevails). A complete list of CLI options is available running benchmarks with the {\ttfamily -\/-\/help} switch.

\label{_running-a-subset-of-benchmarks}%
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md39}{}\doxysection{Running a Subset of Benchmarks}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md39}
The {\ttfamily -\/-\/benchmark\+\_\+filter=$<$regex$>$} option (or {\ttfamily BENCHMARK\+\_\+\+FILTER=$<$regex$>$} environment variable) can be used to only run the benchmarks that match the specified {\ttfamily $<$regex$>$}. For example\+:
...
\end{DoxyCode}
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md40}{}\doxysection{Disabling Benchmarks}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md40}
It is possible to temporarily disable benchmarks by renaming the benchmark function to have the prefix \char`\"{}\+DISABLED\+\_\+\char`\"{}. This will cause the benchmark to be skipped at runtime.

\label{_result-comparison}%
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md41}{}\doxysection{Result comparison}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md41}
It is possible to compare the benchmarking results. See \mbox{\hyperlink{md_build__deps_googlebenchmark_src_docs_tools}{Additional Tooling Documentation}}

\label{_extra-context}%
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md42}{}\doxysection{Extra Context}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md42}
Sometimes it\textquotesingle{}s useful to add extra context to the content printed before the results. By default this section includes information about the CPU on which the benchmarks are running. If you do want to add more context, you can use the {\ttfamily benchmark\+\_\+context} command line flag\+:


\begin{DoxyCode}{0}
\DoxyCodeLine{\$ ./run\_benchmarks -\/-\/benchmark\_context=pwd=`pwd`}
\DoxyCodeLine{Run on (1 x 2300 MHz CPU)}
\DoxyCodeLine{pwd: /home/user/benchmark/}
\DoxyCodeLine{Benchmark              Time           CPU Iterations}
\DoxyCodeLine{-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/-\/}
\DoxyCodeLine{BM\_memcpy/32          11 ns         11 ns   79545455}
\DoxyCodeLine{BM\_memcpy/32k       2181 ns       2185 ns     324074}
...
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md43}{}\doxysection{Runtime and Reporting Considerations}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md43}
...
Furthermore warming up a benchmark might be necessary in order to get stable results because of e.\+g caching effects of the code under benchmark. Warming up means running the benchmark a given amount of time, before results are actually taken into account. The amount of time for which the warmup should be run can be set per benchmark by calling {\ttfamily Min\+Warm\+Up\+Time} on the registered benchmark object or for all benchmarks using the {\ttfamily -\/-\/benchmark\+\_\+min\+\_\+warmup\+\_\+time} command-\/line option. Note that {\ttfamily Min\+Warm\+Up\+Time} will overwrite the value of {\ttfamily -\/-\/benchmark\+\_\+min\+\_\+warmup\+\_\+time} for the single benchmark. How many iterations the warmup run of each benchmark takes is determined the same way as described in the paragraph above. Per default the warmup phase is set to 0 seconds and is therefore disabled.
...
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md44}{}\doxysection{Setup/\+Teardown}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md44}
...
\begin{DoxyItemize}
\item BM\+\_\+func\+\_\+\+Arg\+\_\+1\+\_\+\+Threads\+\_\+16, BM\+\_\+func\+\_\+\+Arg\+\_\+1\+\_\+\+Threads\+\_\+32
\item BM\+\_\+func\+\_\+\+Arg\+\_\+3\+\_\+\+Threads\+\_\+16, BM\+\_\+func\+\_\+\+Arg\+\_\+3\+\_\+\+Threads\+\_\+32
\end{DoxyItemize}

\label{_passing-arguments}%
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md45}{}\doxysection{Passing Arguments}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md45}
...
\DoxyCodeLine{\}}
\DoxyCodeLine{BENCHMARK(BM\_memcpy)-\/>Arg(8)-\/>Arg(64)-\/>Arg(512)-\/>Arg(4<<10)-\/>Arg(8<<10);}
...
\DoxyCodeLine{    auto data = v.data();}
\DoxyCodeLine{    benchmark::DoNotOptimize(data);}
...
\end{DoxyCode}
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md46}{}\doxysubsection{Passing Arbitrary Arguments to a Benchmark}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md46}
In C++11 it is possible to define a benchmark that takes an arbitrary number of extra arguments. The {\ttfamily BENCHMARK\+\_\+\+CAPTURE(func, test\+\_\+case\+\_\+name, ...\+args)} macro creates a benchmark that invokes {\ttfamily func} with the {\ttfamily \mbox{\hyperlink{classbenchmark_1_1State}{benchmark\+::\+State}}} as the first argument followed by the specified {\ttfamily args...}. The {\ttfamily test\+\_\+case\+\_\+name} is appended to the name of the benchmark and should describe the values passed.


\begin{DoxyCode}{0}
\DoxyCodeLine{ \{c++\}}
\DoxyCodeLine{template <class ...Args>}
\DoxyCodeLine{void BM\_takes\_args(benchmark::State\& state, Args\&\&... args) \{}
\DoxyCodeLine{  auto args\_tuple = std::make\_tuple(std::move(args)...);}
\DoxyCodeLine{  for (auto \_ : state) \{}
\DoxyCodeLine{    std::cout << std::get<0>(args\_tuple) << "{}: "{} << std::get<1>(args\_tuple)}
\DoxyCodeLine{              << '\(\backslash\)n';}
\DoxyCodeLine{    [...]}
\DoxyCodeLine{  \}}
\DoxyCodeLine{\}}
\DoxyCodeLine{// Registers a benchmark named "{}BM\_takes\_args/int\_string\_test"{} that passes}
\DoxyCodeLine{// the specified values to `args`.}
\DoxyCodeLine{BENCHMARK\_CAPTURE(BM\_takes\_args, int\_string\_test, 42, std::string("{}abc"{}));}
\DoxyCodeLine{}
\DoxyCodeLine{// Registers the same benchmark "{}BM\_takes\_args/int\_test"{} that passes}
\DoxyCodeLine{// the specified values to `args`.}
\DoxyCodeLine{BENCHMARK\_CAPTURE(BM\_takes\_args, int\_test, 42, 43);}
...
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md47}{}\doxysection{Calculating Asymptotic Complexity (\+Big O)}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md47}
...
\DoxyCodeLine{  for (auto \_ : state) \{}
\DoxyCodeLine{    auto comparison\_result = s1.compare(s2);}
\DoxyCodeLine{    benchmark::DoNotOptimize(comparison\_result);}
\DoxyCodeLine{  \}}
...
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md48}{}\doxysection{Custom Benchmark Name}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md48}
...
The invocation will execute the benchmark as before using {\ttfamily BM\+\_\+memcpy} but changes the prefix in the report to {\ttfamily memcpy}.

\label{_templated-benchmarks}%
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md49}{}\doxysection{Templated Benchmarks}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md49}
...
\label{_fixtures}%
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md50}{}\doxysection{Fixtures}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md50}
\mbox{\hyperlink{classFixture}{Fixture}} tests are created by first defining a type that derives from {\ttfamily \mbox{\hyperlink{classbenchmark_1_1Fixture}{benchmark\+::\+Fixture}}} and then creating/registering the tests using the following macros\+:


\begin{DoxyItemize}
\item {\ttfamily \mbox{\hyperlink{benchmark_8h_a2884177eef37d9ba4aff427cb3be186c}{BENCHMARK\+\_\+\+F(\+Class\+Name, Method)}}}
\item {\ttfamily \mbox{\hyperlink{benchmark_8h_a0039ddd9b71ced4dc28e87a932e06b72}{BENCHMARK\+\_\+\+DEFINE\+\_\+\+F(\+Class\+Name, Method)}}}
\item {\ttfamily \mbox{\hyperlink{benchmark_8h_a5f8e8e48e0fec94b83fc52c3b24b5dc0}{BENCHMARK\+\_\+\+REGISTER\+\_\+\+F(\+Class\+Name, Method)}}}
\end{DoxyItemize}
...
\end{DoxyCode}
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md51}{}\doxysubsection{Templated Fixtures}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md51}
Also you can create templated fixture by using the following macros\+:


\begin{DoxyItemize}
\item {\ttfamily \mbox{\hyperlink{benchmark_8h_a1c0a14f1f34985b252eaf093c8828d3e}{BENCHMARK\+\_\+\+TEMPLATE\+\_\+\+F(\+Class\+Name, Method, ...)}}}
\item {\ttfamily \mbox{\hyperlink{benchmark_8h_ad8e970a25f724bab42271e1e37b78daa}{BENCHMARK\+\_\+\+TEMPLATE\+\_\+\+DEFINE\+\_\+\+F(\+Class\+Name, Method, ...)}}}
\end{DoxyItemize}
...
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md52}{}\doxysection{Custom Counters}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md52}
...
\end{DoxyCode}
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md53}{}\doxysubsection{Counter Reporting}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md53}
...
Note above the additional header printed when the benchmark changes from {\ttfamily BM\+\_\+\+User\+Counter} to {\ttfamily BM\+\_\+\+Factorial}. This is because {\ttfamily BM\+\_\+\+Factorial} does not have the same counter set as {\ttfamily BM\+\_\+\+User\+Counter}.

\label{_multithreaded-benchmarks}%
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md54}{}\doxysection{Multithreaded Benchmarks}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md54}
In a multithreaded test (benchmark invoked by multiple threads simultaneously), it is guaranteed that none of the threads will start until all have reached the start of the benchmark loop, and all will have finished before any thread exits the benchmark loop. (This behavior is also provided by the {\ttfamily Keep\+Running()} API) As such, any global setup or teardown can be wrapped in a check against the thread index\+:
...
To run the benchmark across a range of thread counts, instead of {\ttfamily Threads}, use {\ttfamily Thread\+Range}. This takes two parameters ({\ttfamily min\+\_\+threads} and {\ttfamily max\+\_\+threads}) and runs the benchmark once for values in the inclusive range. For example\+:


\begin{DoxyCode}{0}
\DoxyCodeLine{ \{c++\}}
\DoxyCodeLine{BENCHMARK(BM\_MultiThreaded)-\/>ThreadRange(1, 8);}

\end{DoxyCode}


will run {\ttfamily BM\+\_\+\+Multi\+Threaded} with thread counts 1, 2, 4, and 8.
...
Without {\ttfamily Use\+Real\+Time}, CPU time is used by default.

\label{_cpu-timers}%
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md55}{}\doxysection{CPU Timers}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md55}
...
\end{DoxyCode}
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md56}{}\doxysubsection{Controlling Timers}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md56}
...
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md57}{}\doxysection{Manual Timing}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md57}
For benchmarking something for which neither CPU time nor real-\/time are correct or accurate enough, completely manual timing is supported using the {\ttfamily Use\+Manual\+Time} function.
...
An example use case for this is benchmarking GPU execution (e.\+g. Open\+CL or CUDA kernels, Open\+GL or Vulkan or Direct3D draw calls), which cannot be accurately measured using CPU time or real-\/time. Instead, they can be measured accurately using a dedicated API, and these measurement results can be reported back with {\ttfamily Set\+Iteration\+Time}.
...
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md58}{}\doxysection{Setting the Time Unit}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md58}
...
Additionally the default time unit can be set globally with the {\ttfamily -\/-\/benchmark\+\_\+time\+\_\+unit=\{ns$\vert$us$\vert$ms$\vert$s\}} command line argument. The argument only affects benchmarks where the time unit is not set explicitly.

\label{_preventing-optimization}%
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md59}{}\doxysection{Preventing Optimization}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md59}
...
\DoxyCodeLine{    auto data = v.data();           // Allow v.data() to be clobbered. Pass as non-\/const}
\DoxyCodeLine{    benchmark::DoNotOptimize(data); // lvalue to avoid undesired compiler optimizations}
...
Note that {\ttfamily Clobber\+Memory()} is only available for GNU or MSVC based compilers.

\label{_reporting-statistics}%
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md60}{}\doxysection{Statistics\+: Reporting the Mean, Median and Standard Deviation / Coefficient of variation of Repeated Benchmarks}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md60}
...
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md61}{}\doxysection{Custom Statistics}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md61}
...
\DoxyCodeLine{  \}, benchmark::StatisticUnit::kPercentage)}
\DoxyCodeLine{  -\/>Arg(512);}
...
\label{_memory-usage}%
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md62}{}\doxysection{Memory Usage}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md62}
It\textquotesingle{}s often useful to also track memory usage for benchmarks, alongside CPU performance. For this reason, benchmark offers the {\ttfamily Register\+Memory\+Manager} method that allows a custom {\ttfamily Memory\+Manager} to be injected.

If set, the {\ttfamily Memory\+Manager\+::\+Start} and {\ttfamily Memory\+Manager\+::\+Stop} methods will be called at the start and end of benchmark runs to allow user code to fill out a report on the number of allocations, bytes used, etc.
...
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md63}{}\doxysection{Using Register\+Benchmark(name, fn, args...)}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md63}
...
Unlike the {\ttfamily BENCHMARK} registration macros, which can only be used at the global scope, the {\ttfamily Register\+Benchmark} can be called anywhere. This allows for benchmark tests to be registered programmatically.
...
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md64}{}\doxysection{Exiting with an Error}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md64}
When errors caused by external influences, such as file I/O and network communication, occur within a benchmark the {\ttfamily State\+::\+Skip\+With\+Error(const std\+::string\& msg)} function can be used to skip that run of benchmark and report the error. Note that only future iterations of the {\ttfamily Keep\+Running()} are skipped. For the ranged-\/for version of the benchmark loop Users must explicitly exit the loop, otherwise all iterations will be performed. Users may explicitly return to exit the benchmark immediately.
...
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md65}{}\doxysection{A Faster Keep\+Running Loop}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md65}
...
\hypertarget{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md66}{}\doxysection{Disabling CPU Frequency Scaling}\label{md_build__deps_googlebenchmark_src_docs_user_guide_autotoc_md66}
If you see this error\+:


\begin{DoxyCode}{0}
\DoxyCodeLine{***WARNING*** CPU scaling is enabled, the benchmark real time measurements may}
\DoxyCodeLine{be noisy and will incur extra overhead.}
